...
def motion_map(xt, ut, xt_1, m, motion_model, prob, map_model):
    return motion_model(xt, ut, xt_1, prob) * map_model(xt, m)

def sample_motion_map(ut, xt_1, m, sample_motion):
    xt=sample_motion(ut, xt_1)
    pi_const= map_model(xt, m)   
    while(pi_const < 0) :
        xt=sample_motion(ut, xt_1)
        pi_const= map_model(xt, m)
    return pi_const
